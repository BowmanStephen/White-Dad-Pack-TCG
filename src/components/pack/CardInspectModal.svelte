...
// Only handle keys when modal is actually open (card is not null)
function handleKeydown(e: KeyboardEvent) {
  // Guard: only handle keys when modal is open
  if (!card) return;
...
{#if card}
  ...
  <div
    ...
    class="fixed inset-0 z-[5000] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md"
    ...
    tabindex="-1"
  >
    <button class="absolute inset-0" onclick={onClose} aria-label="Close card details" type="button"
    ></button>
    <!-- Close button -->
    <button
      onclick={e => {
      ...
      class="fixed top-8 right-8 text-white/50 hover:text-white text-5xl font-light transition-colors z-[5001]"
    ...
    >
      <div
        class="flex flex-col lg:flex-row gap-12 items-center lg:items-center px-4 md:px-20 pb-10"
      >
        <!-- Card display -->
        ...
        <div class="flex-1 w-full text-white">
          ...
          {#if card.flavorText}
            <blockquote
              class="text-slate-300 italic mb-10 text-2xl leading-relaxed opacity-80 border-l-8 pl-8"
          ...
          <div class="grid grid-cols-2 gap-4 mb-10">
            <div class="bg-white/5 rounded-2xl p-6 border border-white/10">
              <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">
                Identity
              </h3>
              <div class="flex items-center gap-3">
                <span class="text-4xl">{DAD_TYPE_ICONS[card.type]}</span>
                <div>
                  <div class="font-black text-white uppercase text-sm tracking-widest">
                    {card.type.replace('_', ' ')}
                  </div>
          ...
          <div class="flex flex-wrap gap-4 mt-auto opacity-40">
            <span class="text-[10px] font-bold uppercase tracking-widest"
              >Artist: {card.artist}</span
            >
            <span class="text-[10px] font-bold uppercase tracking-widest"
              >Series: {card.series}</span
            >
