...
<div class="w-full max-w-4xl mx-auto pb-20 px-4">
  ...
  <div class="mb-12" in:fly={{ y: 30, duration: 600, delay: 500 }}>
    ...
    >
      <div class="flex items-center gap-4">
        <div class="w-12 h-12 rounded-full flex items-center justify-center border-2" style="background: linear-gradient(135deg, {packQuality.grade.color}22, {packQuality.grade.color}44); border-color: {packQuality.grade.color};">
          <span class="text-xl font-black" style="color: {packQuality.grade.color};">{packQuality.grade.grade}</span>
        </div>
        <div class="text-left">
          <div class="text-white font-bold">Pack Grade: {packQuality.grade.label}</div>
          <div class="text-slate-400 text-sm">{packQuality.score} / {packQuality.maxPossibleScore} points</div>
        </div>
      </div>
      <svg class="w-5 h-5 text-slate-400 transition-transform" class:rotate-180={showDetails} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <polyline points="6 9 12 15 18 9"></polyline>
      </svg>
    </button>

    {#if showDetails}
      <div class="mt-2 p-6 bg-slate-900/50 rounded-2xl border border-white/5 backdrop-blur-sm" in:fly={{ y: -10, duration: 200 }}>
  ...
  <div class="mb-16 grid grid-cols-1 md:grid-cols-2 gap-6" in:fade={{ duration: 600, delay: 1100 }}>
      ...
      <div class="flex flex-wrap gap-3">
        {#each rarityCounts as { rarity, count, config }}
          <div class="flex items-center gap-3 bg-slate-800/40 px-4 py-2.5 rounded-xl border border-white/5 transition-colors hover:bg-slate-800/60">
            <div class="w-3 h-3 rounded-full shadow-[0_0_10px_currentColor]" style="background: {config.color}; color: {config.color};"></div>
    ...
    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-4 md:gap-6">
      {#each sortedCards as card, i}
        ...
        >
          ...
          <div class="absolute bottom-0 left-0 right-0 h-1" style="background: {cardRarity.color};"></div>
