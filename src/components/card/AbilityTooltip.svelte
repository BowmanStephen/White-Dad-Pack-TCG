...
let { ability, triggerElement, cardRarity, delay = 500 }: Props = $props();
...
function formatDescription(text: string): string {
  ...
  text = text.replace(
    /(When |Whenever |If |At the beginning of )/gi,
    ...
    >
      ...
      <div class="px-3 py-2.5">
        ...
        >
          ...
          {@html sanitizeHTML(formatDescription(ability.description))}
        ...
        {#if getExamples(ability.description).length > 0}
          <div class="mt-2 pt-2 border-t border-white/10">
            <p class="text-[10px] font-bold text-slate-400 mb-1">
              EXAMPLE{getExamples(ability.description).length > 1 ? 'S' : ''}:
            </p>
            {#each getExamples(ability.description) as example}
              <p
                class="text-[10px] text-slate-300 italic pl-2 border-l-2"
      ...
      <div
        class="absolute top-0 left-0 w-3 h-3 rounded-tl-lg"
        style="background: {rarityConfig.color}44;"
      ></div>
      <div
        class="absolute top-0 right-0 w-3 h-3 rounded-tr-lg"
        style="background: {rarityConfig.color}44;"
      ></div>
      <div
        class="absolute bottom-0 left-0 w-3 h-3 rounded-bl-lg"
        style="background: {rarityConfig.color}44;"
      ></div>
      <div
        class="absolute bottom-0 right-0 w-3 h-3 rounded-br-lg"
        style="background: {rarityConfig.color}44;"
      ></div>
    </div>
  </div>
