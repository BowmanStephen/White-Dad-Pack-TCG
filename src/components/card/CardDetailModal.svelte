  ...
  >
    ...
    <div class="p-8">
      ...
      <div class="text-center mb-6">
        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4" style="background: {rarityConfig.color}22; border: 1px solid {rarityConfig.color}55;">
          ...
          <span class="text-sm font-bold" style="color: {rarityConfig.color};">{rarityConfig.name}</span>
        </div>

        <h1 id="modal-title" class="text-4xl font-black text-white mb-2" style="text-shadow: 0 4px 8px rgba(0,0,0,0.5);">
          {card.name}
        </h1>
      ...
      <div class="flex justify-center mb-8">
        <div class="relative w-[400px] h-[400px] rounded-2xl overflow-hidden shadow-2xl" style="box-shadow: 0 8px 30px rgba(0,0,0,0.6), 0 0 40px {rarityConfig.glowColor}44;">
          <GenerativeCardArt
            card={card}
            width={400}
            height={400}
      ...
      <div class="mb-8">
        <div class="relative bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-xl p-8 border-l-4" style="border-color: {rarityConfig.color};">
          <div class="absolute top-4 left-4 text-6xl opacity-20" style="color: {rarityConfig.color};" aria-hidden="true">"</div>
          <p class="relative text-xl text-slate-200 italic leading-relaxed text-center">
            {card.flavorText}
          </p>
          <div class="absolute bottom-4 right-4 text-6xl opacity-20" style="color: {rarityConfig.color};" aria-hidden="true">"</div>
        </div>
      </div>
          ...
          <div class="grid gap-4">
            {#each card.abilities as ability, index}
              <div class="ability-indicator" bind:this={abilityElements[index]}>
                <div class="bg-black/30 rounded-xl p-5 border border-white/10 hover:border-white/20 transition-all duration-200">
                  <div class="flex items-start gap-3">
                    <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-lg" style="background: {rarityConfig.color}22;">
